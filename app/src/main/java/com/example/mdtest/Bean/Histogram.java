...
import com.example.mdtest.Util.DensityUtils;

public class Histogram extends View {
    int MAX = 100;      //矩形显示的最大值
    int corner = 5;         //矩形的角度,设置为0则没有角度
    double data = 0.0;      //显示的数
    double tempData = 0;        //初始数据
    int textPadding = 15;       //字体与矩形图的距离
    Paint mPaint;
    int mColor;
    Context mContext;
...
//        mPaint.setTypeface(typeface);
        ...
        if (S.length() < 4) {
            ...
            mPaint.setTextSize(40); //可以通过getWidth()/2 改变字体大小 也可以通过设置数字来改变自己想要的字体大小 当超出矩形图宽度时不能显示全部
        }

        float textH = mPaint.ascent() + mPaint.descent();
        float MaxH = getHeight() - textH - 10 * DensityUtils.pxTodip(mContext, textPadding);        //修改数字控制柱状图最大高度
